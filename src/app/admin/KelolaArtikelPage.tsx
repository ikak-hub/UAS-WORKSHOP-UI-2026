 ...
 * Halaman manajemen artikel — menampilkan folder artikel dan tombol aksi.
 */
export default function KelolaArtikelPage({
  ...
  const [folders, setFolders] = useState([
    "FOLDER ARTIKEL ADAT",
    "FOLDER ARTIKEL COSPLAY",
    "FOLDER ARTIKEL WISUDA",
  ]);
    ...
    <div style={{ minHeight: "100vh", background: "#fff", display: "flex", flexDirection: "column" }}>
      ...
      <main style={{ flex: 1, padding: "24px 40px 80px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32, maxWidth: 900, marginBottom: 48 }}>
          {folders.map((f, i) => (
            <FolderIcon key={i} label={f} onDelete={() => deleteFolder(i)} />
          ))}
        </div>
...
// ─── Sub-komponen: Folder Icon ─────────────────────────────────────────────────
function FolderIcon({ label, onDelete }: { label: string; onDelete: () => void }) {
  return (
    <div style={{ position: "relative", cursor: "pointer" }}>
      <div style={{ width: "100%", background: "#F5A623", borderRadius: "0 16px 16px 16px", minHeight: 200, position: "relative", paddingTop: 40 }}>
        <div style={{ position: "absolute", top: -20, left: 0, width: 140, height: 22, background: "#F5A623", borderRadius: "12px 12px 0 0" }} />
        <p style={{ fontFamily: "'Open Sans', sans-serif", fontWeight: 700, fontSize: 14, color: "#000", padding: "0 16px", position: "absolute", top: -35, left: 0 }}>
          {label}
        </p>
